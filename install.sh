...
echo "***Check hostname in opennet/hosts"
for host in $hosts; do
    HOST_STATUS=$(cat ansible/hosts | grep -v "\[opennet-master\]" | grep -v "\[opennet-slave\]" | grep -w $host| awk '{print $1}')
    if [ -z $HOST_STATUS ]; then
        echo "Can NOT find hostname '$host' in ansible/hosts, please check."
        echo "Please appedn hostname '$host' in [opennet-master] or [opennet-slave] sections"
        exit
    fi
done
echo
